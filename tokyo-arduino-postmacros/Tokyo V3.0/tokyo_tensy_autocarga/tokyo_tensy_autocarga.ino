...
bool led_encontrado = false;
float navVxPARKING = 0.0;
float navVthPARKING = 0.0;
bool activar_IRF = false;
int pinIR_DER = 25 ;
int pinIR_IZQ = 24 ;
...
int encoderDERBp = 11;
int encoderDERAn = 12;
int encoderDERBn = 9;//13 EN RUNNER
...
Estado_PARKING estado_PARKING = DESACTIVADO;
Estado_PARKING last_estado_PARKING = DESACTIVADO;
...
static struct pt pt1, pt2, ptREAD, ptSEND, ptPARKING, ptBATTERY, ptCABEZA; // each protothread needs one of these
bool isMoving = false;
...
void setup()
{
  ...
  Serial1.begin(57600);  //MegaCabeza
  Serial2.begin(9600);
  // Serial.begin(115200);
...
void loop() {
  ...
  //--------Aparcamiento para autocarga
  parkingAutocarga(&ptPARKING);
...
static int readNucROS(struct pt * pt) {
  ...
  if (!breaking) {
    if (estado_NAV == PAUSA) {
      ...
      if (navVth > -0.05 || absVx * offset >= absVth) {//absVx >= 0.40
        estado_NAV = PAUSA;
        navegacion();
      }
      else {
...
static int parkingAutocarga(struct pt * pt) {
  static unsigned long timestamp = 0;
  PT_BEGIN(pt);
...
void comprobacionEstados() {
  if (estado_PARKING == DESACTIVADO) {
    battery.data = 0;
    if (isparking) { //Terminada navegación
      estado_PARKING = CENTRARSE;
    }

  }
  else if (estado_PARKING == CENTRARSE) {
    battery.data = 1;

    if (led_encontrado) {
      estado_PARKING = GIRO180;
      resetEncoders();
    }
  }
  else if (estado_PARKING == GIRO180) {
    battery.data = 2;
    navVxPARKING = 0.4;
...
float RampRight(int movido, double tiempo) {
  ...
  if (stepRight < 0.3) {
    stepRight += 0.15 * tiempo;
  } else if (stepRight < 0.6) {
    stepRight += 0.3 * tiempo;
  } else {
    stepRight += 0.5 * tiempo;
  }
  ...
  float rampVelX = lerp(0.00, navVx, stepRight);
  float rampoVelVth = lerp(0.00, navVth, stepRight);

  float vel = abs(rampVelX - (rampoVelVth * distRuedas) / 2);
...
float RampLeft(int movido, double tiempo) {
  ...
  float rampVelX = lerp(0.00, navVx, stepLeft);
  float rampoVelVth = lerp(0.00, navVth, stepLeft);
...
void calculoCinematicaRight() {
  ...
  frec_der = 1000000 / ((abs(currentVelDer) * 800) / 0.5);
  lastRight = current;
}
...
void calculoCinematicaLeft() {
  ...
  frec_izq = 1000000 / ((abs(currentVelIzq) * 800) / 0.5);
  lastLeft = current;
}
  ...
  if (Serial2.available() > 0) {
    {
      ...
      for (int i = 0; i < Length + 2; i++) { // get the checksum in last two bytes, just in case need later
        incomingByte = Serial2.read(); // get the rest of the data, how long it might be.
